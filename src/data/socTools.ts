...
const edrTools: ToolDetail[] = [
  {
    ...
    workflow: [
      '1. **Get the Alarm:** The system sends an alert (like a smoke detector going off).',
      '2. **Check the Video:** Analysts look at the endpoint’s activity timeline (the "video recording") to see exactly what happened.',
      '3. **Search the Building:** Analysts use search tools (FQL) to check if other computers show the same suspicious activity.',
      '4. **Lock the Door:** The analyst remotely isolates the infected computer from the network to stop the spread.',
      '5. **Clean Up:** The analyst kills the malicious program and deletes the malware.',
      '6. **Proactive Patrol:** Threat hunters actively search for hidden threats that haven\'t triggered an alarm yet.',
    ],
  ...
  },
  {
    ...
    workflow: [
      '1. **Auto-Fix:** The system detects a threat and immediately starts fixing it automatically (killing the bad program, cleaning files).',
      '2. **Review the Story:** Analysts check the "Storyline" to confirm the threat was real and see exactly how the attack unfolded.',
      '3. **Time Travel (Rollback):** If files were encrypted or damaged, the analyst uses the rollback feature to restore the computer to its state before the attack.',
      '4. **Check the Fleet:** Analysts search the data to ensure no other computers were affected by the same threat.',
      '5. **Confirm Safety:** The analyst confirms the threat is gone and releases the computer from quarantine.',
    ],
  ...
  },
  {
    ...
    workflow: [
      '1. **Alert & Auto-Investigate:** An alert is generated, and the system\'s AI immediately starts collecting evidence (AIR).',
      '2. **Review AI Findings:** Analysts check the incident graph to see what the AI found and if it successfully neutralized the threat.',
      '3. **Deep Search:** If the threat is complex, analysts use KQL (a powerful search language) to hunt for specific files or IPs across all Microsoft data (email, identity, endpoint).',
      '4. **Remote Control:** Analysts use "Live Response" to remotely connect to the computer and run manual commands to gather more evidence or clean up.',
      '5. **Containment:** The analyst isolates the host if the automated steps were not enough to stop the threat.',
    ],
  ...
  },
  {
    ...
    workflow: [
      '1. **Unified Story:** The system collects data from the computer, the network firewall, and the cloud, then stitches it all together into one single attack timeline.',
      '2. **Triage:** Analysts review the single, correlated incident story, which clearly shows the root cause and MITRE attack steps.',
      '3. **Cross-Domain Hunt:** Analysts use the query language to search for the malicious activity across all correlated data sources (e.g., "Did the firewall see this IP?").',
      '4. **Orchestrated Response:** The analyst triggers a response that affects multiple systems (e.g., isolate the computer AND block the attacker\'s IP on the firewall automatically).',
      '5. **Documentation:** The full, unified story is saved for reporting and future training.',
    ],
  ...
  },
  {
    ...
    workflow: [
      '1. **Alert Review:** Analysts review the alert and the visual process chain to understand the attack context.',
      '2. **Remote Access:** Analysts use the "Live Response" feature to securely connect to the infected computer and run manual commands (like checking a specific file).',
      '3. **Search the Stream:** Analysts use custom queries to search the continuous stream of endpoint events for the initial point of entry.',
      '4. **Containment:** If malicious, the analyst quarantines the host and globally blocks the malicious file across all computers.',
      '5. **Root Cause Report:** A detailed report is generated, focusing on the forensic evidence gathered from the endpoint.',
    ],
  ...
  },
  {
    ...
    workflow: [
      '1. **Custom Alert:** A very specific alert triggers based on a rule the analyst wrote (e.g., "Alert if Notepad tries to access the password file").',
      '2. **Review Raw Data:** Analysts look at the raw, low-level event data to confirm the context of the alert.',
      '3. **Remote Check:** Analysts use a remote command to check the affected computer for immediate verification (e.g., "Show me the contents of that suspicious file").',
      '4. **Global Block:** If confirmed malicious, the analyst instantly deploys a new D&R rule to block that specific behavior across all computers.',
      '5. **Forensics:** The analyst exports the raw event data and collects forensic artifacts (like a memory dump) for deep, offline analysis.',
    ],
